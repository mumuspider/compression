...
def setup_logging(args, config):
    ...
    if args.mode == 'paper':
        ...
        exp_name = f"{image_name}_LoR-SGS+_paper_P{args.num_points}_it{args.iterations}_R{config['rank']}_S{args.seed}"
    else:
        image_name = Path(args.img_path).stem if args.img_path else args.dataset
        exp_name = f"{image_name}_LoR-SGS+_P{args.num_points}_it{args.iterations}_R{config['rank']}_S{args.seed}"
    if results_root:
...
def main():
    ...
    parser.add_argument('--use_coef_refine', action=argparse.BooleanOptionalAction, default=True,
                        help='Enable the coefficient refinement block.')
    parser.add_argument('--coef_refine_kernel', type=int, default=3,
                        help='Kernel size of the coefficient refinement depthwise convolution.')
    parser.add_argument('--use_endmember_calib', action=argparse.BooleanOptionalAction, default=True,
                        help='Enable low-rank endmember calibration.')
    parser.add_argument('--calib_rank', type=int, default=2,
                        help='Rank of the low-rank endmember calibration.')
    ...
    print(f"Use coefficient refinement: {args.use_coef_refine}")
    print(f"Coefficient refinement kernel: {args.coef_refine_kernel}")
    print(f"Use endmember calibration: {args.use_endmember_calib}")
    ...
    metrics, training_logs, checkpoint_paths = train_nd(
        ...
        model_name='GaussianImage_Cholesky_nd_plus',
        image_name=image_name,
        data_name=data_name,
        use_coef_refine=args.use_coef_refine,
        coef_refine_kernel=args.coef_refine_kernel,
        use_endmember_calib=args.use_endmember_calib,
