...
class SimpleTrainerHSI:
    ...
    def __init__(
        ...
        self.use_coef_refine = use_coef_refine
        self.coef_refine_kernel = coef_refine_kernel
        self.use_endmember_calib = use_endmember_calib
        ...
        if model_name == 'GaussianImage_Cholesky_nd' or model_name.startswith('GaussianImage_Cholesky_nd_plus'):
            ...
            self.gaussian_model = GaussianImage_Cholesky_EA(
                ...
                lr=5e-3,
                quantize=False,
                use_coef_refine=self.use_coef_refine,
                coef_refine_kernel=self.coef_refine_kernel,
                use_endmember_calib=self.use_endmember_calib,
                calib_rank=self.calib_rank,
            ).to(self.device)
        else:
    ...
    def test(self, best=False):
        ...
        with torch.no_grad():
            ...
            A = out['render'].float()
            E = FakeQuantizationHalf.apply(self.gaussian_model.get_calibrated_endmember().to(torch.float32))
            I = A @ E
...
def train_nd(
    ...
    trainer = SimpleTrainerHSI(
        ...
        data_name=data_name,
        use_coef_refine=use_coef_refine,
        coef_refine_kernel=coef_refine_kernel,
        use_endmember_calib=use_endmember_calib,
        calib_rank=calib_rank,
    )
