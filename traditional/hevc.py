...
def hevc_compress_hsi(hsi_path, output_dir, crf_levels, device, save_mat):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        task_dir = create_task_output_dir(output_dir, "hevc", hsi_path)
        ...
        for crf in crf_levels:
            print(f"\nHEVC - 目标 CRF: {crf}")
            video_path = task_dir / f"{image_name}_hevc_crf{crf}.mp4"
            ...
            if save_mat:
                output_mat = task_dir / f"{image_name}_hevc_crf{crf}.mat"
                sio.savemat(output_mat, {'data': rec_hsi_data.transpose(1, 2, 0)})

        json_path = task_dir / f"{image_name}_hevc_results.json"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--crf_levels', type=str, default='18,21,23,26,28,31,33,36,39,42', help='逗号分隔的HEVC CRF值列表')
